...
bool L_NAMOA_DR_MVH::better_local_dominance_check(
    ...
    for (const auto& pareto_elem : pareto_list[id]) {
        ...
        if (dom) {
            // here - bad fallback, would have returned true anyway
            num_bad_fallback++;
            return true;
        }
    }
...
bool L_NAMOA_DR_MVH::global_dominance_check(const NodePtr& node_ptr, const size_t& target_id) {
...
L_NAMOA_DR_MVH::get_first_undominated_heuristic_value(
...
void L_NAMOA_DR_MVH::operator()(
    ...
    // Compute average sizes of truncated_non_dominated_g and pareto_list over active states only
    size_t total_truncated = 0;
    size_t total_pareto = 0;
    num_active_states = 0;
    size_t num_nodes = truncated_non_dominated_g.size();
    for (size_t i = 0; i < num_nodes; i++) {
        if (pareto_list[i].size() > 0 || truncated_non_dominated_g[i].size() > 0) {
            num_active_states++;
