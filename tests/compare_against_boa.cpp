...
RunResult run_boa(const AdjacencyMatrix& graph,
                  const QueryCase& query,
                  const Heuristic& heuristic,
                  const std::string& name) {
    BOAStar solver(graph);
...
bool compare_to_boa(const QueryCase& query,
                    const RunResult& boa,
                    const RunResult& candidate) {
    const bool matches = boa.costs == candidate.costs && !candidate.time_limit_reached;
    if (matches) {
    ...
    std::cout << "  FAIL " << candidate.name << "\n"
              ...
              << " target=" << query.target << "\n"
              << "    BOA* solutions=" << boa.costs.size()
              << " candidate solutions=" << candidate.costs.size()
              << " candidate_time_limit_reached=" << candidate.time_limit_reached << "\n";
    print_set_difference(boa.costs, candidate.costs, "missing vs BOA*");
    print_set_difference(candidate.costs, boa.costs, "extra vs BOA*");
    return false;
}
...
int main(int argc, char** argv) {
    ...
    const std::vector<QueryCase> cases = {
        {"distance_time_1_to_20", {0, 4}, 1, 20},
        {"distance_length_10_to_77", {0, 2}, 10, 77},
        {"difficulty_elevation_43_to_96", {1, 3}, 43, 96},
    };

    bool all_ok = true;
    for (const auto& query : cases) {
        std::cout << "\n=== " << query.name
                  << " objectives=(" << query.objectives[0] << ", "
                  << query.objectives[1] << ") ===\n";
        ...
        const RunResult boa = run_boa(graph, query, sp, "BOA* + shortest-path heuristic");
        std::cout << "  REF BOA* solutions=" << boa.costs.size()
                  << " costs=" << format_cost_set(boa.costs) << "\n";
        ...
        const std::vector<RunResult> candidates = {
            run_boa(graph, query, zero, "BOA* + zero heuristic"),
        ...
        for (const auto& candidate : candidates) {
            all_ok = compare_to_boa(query, boa, candidate) && all_ok;
        }
    }

    if (!all_ok) {
        std::cout << "\nOne or more synthetic graph BOA* comparison checks failed.\n";
        return 1;
    }

    std::cout << "\nAll synthetic graph BOA* comparison checks passed.\n";
    return 0;
}
